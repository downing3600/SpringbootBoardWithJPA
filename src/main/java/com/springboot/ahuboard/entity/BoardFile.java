...
public class BoardFile {
	...
	@Column
	private long size;// 업로드 된 파일의 크기
	
	@CreationTimestamp
	private Timestamp uploadTime; // 업로드 시기
	
	@UpdateTimestamp
	private Timestamp confirmTime; // 최정저장 시기
	
	@ManyToOne(targetEntity = Board.class)//게시글과 N:1 관계
	@JoinColumn(name= "board_no")//외래키 저장 컬럼명 설정
	private Board board; //게시글 번호 (최종 저장 후 설정 가능)
